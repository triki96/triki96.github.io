...
categories: [Cyber Security 101, linux-fundamentals]
tags: [linux, linux-fundamentals, shell, directories, passwd]
...
**systemd** è il processo con PID 1, il primo che il kernel Linux avvia quando la macchina si accende, da cui derivano direttamente o indirettamente tutti gli altri processi del sistema. È un insieme di componenti che lavorano insieme: `systemd` (il processo PID 1, gestisce avvio e supervisione dei servizi), `systemd-journald` (gestisce i log), `systemd-logind` (gestisce le sessioni utente), `systemd-networkd`, `systemd-resolved`, e altri componenti specializzati. In pratica, è "il motore" che gira sempre in background, gestendo attivamente il ciclo di vita dei servizi.

**systemctl** è lo strumento a riga di comando con cui si parla con systemd, gli si danno istruzioni e si chiedono informazioni. Non fa nulla da solo: comunica con il processo systemd e traduce i comandi in richieste che systemd esegue effettivamente.
