...
Il filo conduttore di tutti gli strumenti `*2john` (`zip2john`, `rar2john`, `ssh2john`, e allo stesso modo `gpg2john`) è sempre lo stesso: John non attacca mai il file originale direttamente, serve sempre un passaggio intermedio che **estragga** l'hash/verifica interna in un formato standardizzato che John sappia processare. Questo pattern ricorrente vale la pena ricordarlo: qualunque sia il file protetto da password che si incontra durante una CTF o un assessment (un archivio, una chiave privata, un documento cifrato), la prima domanda da porsi è "esiste uno strumento `*2john` per questo formato?" prima di cercare altre strade.
