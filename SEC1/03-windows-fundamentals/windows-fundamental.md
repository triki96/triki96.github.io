...
tags: [windows-fundamentals, windows, bitlocker, TPM, VSS]
description: "Windows Update, Windows Security, BitLocker/TPM e VSS."
toc: true
---
...
Windows Update è probabilmente il meccanismo di difesa più importante e sottovalutato: molte delle vulnerabilità esistono e vengono sfruttate proprio perché un sistema non è stato aggiornato in tempo. In un contesto di pentesting, verificare se una macchina target ha aggiornamenti mancanti è uno dei primi passi di enumerazione, perché spesso rivela esattamente quali CVE note potrebbero funzionare contro quel sistema.

## Windows Security

Windows Security è il pannello centrale da cui Windows mostra e gestisce le proprie difese, tutte in un unico posto: controlla se ci sono virus o minacce (antivirus integrato), gestisce il firewall che decide quale traffico di rete far passare, verifica la reputazione di app e siti prima di aprirli, e protegge il sistema a livello hardware (verificando che l'avvio non sia stato manomesso). In pratica, è il "cruscotto" da cui capire a colpo d'occhio se il computer è protetto o se qualcosa richiede attenzione.
...
È rilevante per la sicurezza per un motivo semplice: gli attaccanti (soprattutto i ransomware) cercano di cancellare le shadow copies prima di cifrare i file, per impedire che la vittima recuperi tutto semplicemente ripristinando una versione precedente.
